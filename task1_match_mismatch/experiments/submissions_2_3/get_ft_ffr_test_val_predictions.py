...
def get_predictions(model, test_dict):
    ...
    for recording, ds_test in test_dict.items():
        ...
        np.savez(f'./task1_match_mismatch/experiments/results_submission_3/predictions_finetuned_ffr_test_val/{recording}_predictions.npz', predictions=predictions, targets=targets)
...
if __name__ == "__main__":
    ...
    finetuned_ffr_models = glob.glob('./task1_match_mismatch/experiments/results_submission_3/heldout_predictions_finetuned_ffr/*.json')
    finetuned_ffr_models = [x for x in finetuned_ffr_models if 'outputs' not in x]
    finetuned_basenames = sorted([os.path.basename(x) for x in finetuned_ffr_models])
    ...
    for subject in sorted(finetuned_subjects):
        ...
        model = tf.keras.models.load_model(f'./task1_match_mismatch/experiments/results_submission_3/finetuned_ffr_models/finetuned_{subject}.h5')
