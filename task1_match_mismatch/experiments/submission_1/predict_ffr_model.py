...
import json
import numpy as np
from task1_match_mismatch.util import modulations
...
if __name__ == '__main__':
    ...
    # Calculate high-frequency envelope modulations of the speech files (only if the envelope directory does not exist)
    stimuli_dir = os.path.join(dataset_dir, 'stimuli_segments')
    env_mods_dir = os.path.join(dataset_dir, 'modulation_segments')
    # Create envelope of segments if it has not already been created
    if not os.path.isdir(env_mods_dir):
        os.makedirs(env_mods_dir, exist_ok=True)
    for stimulus_seg in glob.glob(os.path.join(stimuli_dir, '*.npz')):
        base_name = os.path.basename(stimulus_seg).split('.')[0]

        ## up to here
        if not os.path.exists(os.path.join(env_mods_dir, base_name + '.npz')):
            mod = modulations.calculate_env_modulations(stimulus_seg)
            target_path = os.path.join(env_mods_dir, base_name + '.npz')
            np.savez(target_path, envelope=mod)
    ...
    for sub_path in test_data:
        ...
        prediction_dir = os.path.join(os.path.dirname(__file__), '../', 'results_submission_1', 'heldout_predictions_avg_ffr')
