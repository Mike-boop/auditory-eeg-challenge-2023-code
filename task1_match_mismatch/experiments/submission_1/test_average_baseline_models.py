...
dir = './task1_match_mismatch/experiments/results_submission_1/trained_baseline_models/'
...
for mdl in range(1, 51):
    file = os.path.join(dir, f'eval_baseline_{mdl}.json')
    dic = json.load(open(file, 'r'))
    print(np.mean([dic[k]['acc'] for k in dic]))

    mdls.append(
        tf.keras.models.load_model(os.path.join(dir, f'model_env_baseline_{mdl}.h5'))
    )
...
test_files = [x for x in glob.glob(os.path.join('./data/split_data/', "test_-_*")) if os.path.basename(x).split("_-_")[-1].split(".")[0] in ['eeg', 'envelope']]
...
for sub in subjects:
    ...
    test_generator = MatchMismatchDataGenerator(files_test_sub, 64*3, spacing=64)
    datasets_test[sub] = create_tf_dataset(test_generator, 64*3, default_batch_equalizer_fn, 64, 1, feature_dims=(64, 1, 1))
