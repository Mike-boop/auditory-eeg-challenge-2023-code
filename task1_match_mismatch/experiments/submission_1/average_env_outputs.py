...
def get_predictions(model, test_dict):
    ...
    for recording, ds_test in test_dict.items():
        ...
        np.savez(f'./task1_match_mismatch/experiments/results_submission_1/test_predictions_avg_baseline/{recording}_predictions.npz', predictions=predictions, targets=targets)
...
if __name__ == "__main__":
    ...
    for mdl in range(1, 51):
        ...
        mdls.append(
            tf.keras.models.load_model(os.path.join('./task1_match_mismatch/experiments/results_submission_1/trained_baseline_models/', f'model_env_baseline_{mdl}.h5'))
        )
